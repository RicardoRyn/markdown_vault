...
（可选）可以手动在`pyproject.toml`中添加`ruff`的规则:
...
> 注意：是否存在`__init__.py`文件是一个文件夹是否被识别为模块的关键
...
确保`pyproject.toml`中包括`[build-system]`部分，否则无法通过`uv build`进行构建
